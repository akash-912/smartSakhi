...
export function SafeSpacePage({ userName }) {
  ...
  const fetchData = async () => {
    ...
    // NEW: Fetch the current user's past likes and save them to React State!
    ...
    if (likesData) {
      // Map the database rows into a Javascript Set of IDs
      const likedIds = new Set(likesData.map(item => item.reply_id));
        ...
        <div className="grid lg:grid-cols-3 gap-8">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <Card className="p-4 sm:p-6 border-border shadow-sm bg-card rounded-3xl">
                <textarea
                  ...
                  className="w-full resize-none border-none focus:ring-0 text-foreground bg-muted/50 p-4 rounded-2xl mb-4 placeholder-muted-foreground/70 transition-colors focus:bg-muted/70 outline-none"
                  rows={3}
                />
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                  ...
                  <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.95 }} className="w-full sm:w-auto">
                    <Button onClick={handlePostSubmit} disabled={!newPost.trim()} className="w-full sm:w-auto bg-indigo-500 hover:bg-indigo-600 dark:bg-indigo-600 dark:hover:bg-indigo-700 text-white rounded-xl gap-2 px-6 border-none disabled:opacity-50 transition-all">
              ...
              {posts.length === 0 ? (
                ...
                <AnimatePresence>
                  {posts.map((post) => (
                    <motion.div 
                      key={post.id} 
                      layout // This makes the posts gracefully slide when one is deleted
                    ...
                    >
                      {/* Main Post Bubble */}
                      <Card className={`p-5 sm:p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-none rounded-3xl border-transparent ${post.is_anonymous ? 'bg-indigo-50/50 dark:bg-indigo-900/10' : 'bg-card border-border'}`}>
                      ...
                      <AnimatePresence>
                        {post.community_replies && post.community_replies.length > 0 && (
                          <motion.div 
                            ...
                            className="ml-8 sm:ml-12 space-y-3 border-l-2 border-indigo-100 dark:border-indigo-900/50 pl-4"
                          >
                            <AnimatePresence>
                              {post.community_replies.map(reply => (
                                <motion.div 
                                  ...
                                  exit={{ opacity: 0, scale: 0.9, transition: { duration: 0.2 } }}
                                  className="bg-emerald-50 dark:bg-emerald-900/20 p-4 rounded-2xl rounded-tl-none inline-block max-w-[90%] border border-emerald-100 dark:border-emerald-900/30 group"
                                >
                                  ...
                                  <p className="text-emerald-950 dark:text-emerald-100/90 text-sm mb-2 whitespace-pre-wrap">{reply.content}</p>
                                  
                                  {/* ONLY show the Like button if the author is NOT the AI */}
                      ...
                      <AnimatePresence>
                        {activeReplyId === post.id && (
                          <motion.div 
                            ...
                            className="ml-8 sm:ml-12 flex gap-2 overflow-hidden"
                          >
                            <input 
                              type="text" 
                              autoFocus
                              placeholder="Offer some support..." 
                              className="flex-1 bg-background border border-border focus:ring-2 focus:ring-indigo-500/50 rounded-xl px-4 py-2 text-sm shadow-sm text-foreground placeholder-muted-foreground outline-none transition-shadow"
                              value={replyText}
          ...
          <div className="space-y-6">
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="sticky top-24">
              <Card className="p-6 border-border shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-none bg-gradient-to-br from-orange-50 to-amber-50 dark:from-orange-950/20 dark:to-amber-900/10 rounded-3xl">
                    ...
                    <AnimatePresence>
                      {leaderboard.map((user, index) => (
                        <motion.div 
                          ...
                          className="flex items-center justify-between p-3 bg-white/60 dark:bg-card/50 rounded-2xl backdrop-blur-sm border border-white/40 dark:border-border/50"
                        >
                          <div className="flex items-center gap-3">
                          ...
                          <div className="flex items-center gap-1">
                            <motion.span 
                              key={user.points} // Animate when points change
